...
static void processUplink(uint8_t slotIdx, const uint8_t* buf, int16_t rssi) {
    ...
    Serial.printf("[GW-UL] Slot%d V=%.1f A=%.3f W=%.1f Wh=%lu RSSI=%d\n",
        pkt->nodeId,
        pkt->voltage / 10.0f,
        pkt->current / 1000.0f,
        pkt->power   / 10.0f,
...
static void sendBeacon() {
    ...
    if (st != RADIOLIB_ERR_NONE) {
        Serial.printf("[GW-BCN] TX error %d\n", st);
    }
}
...
static bool sendDownlink(uint8_t slotIdx) {
    ...
    if (ok) {
        ...
        Serial.printf("[GW-DL] Slot%d type=0x%02X len=%d\n",
            slotIdx + 1, ns->queuedCmd.data[0], ns->queuedCmd.len);
    } else {
        Serial.printf("[GW-DL] TX error %d for slot%d\n", st, slotIdx + 1);
    }
    return ok;
}
...
static void handleContentionWindow(uint32_t sfStart) {
    ...
    if (len == (int16_t)sizeof(JoinRequestPacket) && buf[0] == PKT_JOIN_REQUEST) {
        ...
        Serial.printf("[GW-CW] JoinReq UID=0x%04X fw=%d\n", uid, req->fwVersion);
            ...
            if (targetSlot == 0) {
                for (uint8_t i = 0; i < MAX_NODES; i++) {
                    if (!g_nodes[i].active) {
                        ...
                        g_slotMask |= (1u << i);
                        targetSlot  = i + 1;
                        Serial.printf("[GW-CW] Assigned slot %d to UID=0x%04X\n",
                                      targetSlot, uid);
                        break;
                    }
                }
        ...
        if (targetSlot > 0) {
            ...
            Serial.printf("[GW-CW] JoinACK slot=%d\n", targetSlot);
        }
    }
