...
static void loadNodeNames() {
    if (!LittleFS.exists("/node_names.json")) {
        ...
        else   { Serial.println("[WEB] WARN: could not create node_names.json"); }
...
void webServerSetup() {
    ...
    // Registers /wifi_config.html, /api/info, /api/scan, /api/connect,
    // /api/wifistatus, /api/disconnect, /api/forget.
    ...
    server.onNotFound([](AsyncWebServerRequest *req) {
        if (req->method() == HTTP_OPTIONS) {
            ...
            // Redirect OS captive-portal probes → /wifi_config.html when AP on
